  ...
  const fetcher = useCallback(
    async () =>
      getMessages({
        ...
        order: params.order,
        searchTermKeys: ['content.contains', 'platform.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MessageInterface>>(
    () => `/messages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    { id: 'content', header: 'Content', accessorKey: 'content' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.timestamp ? format(parseISO(record?.timestamp as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'platform', header: 'Platform', accessorKey: 'platform' },
    { id: 'status', header: 'Status', accessorKey: 'status' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_message_sender_idTouser',
          header: 'User Message Sender Id Touser',
          accessorKey: 'user_message_sender_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_message_sender_idTouser?.id}`}
            >
              {record.user_message_sender_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_message_receiver_idTouser',
          header: 'User Message Receiver Id Touser',
          accessorKey: 'user_message_receiver_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_message_receiver_idTouser?.id}`}
            >
              {record.user_message_receiver_idTouser?.email}
            </Link>
          ),
        }
      : null,
