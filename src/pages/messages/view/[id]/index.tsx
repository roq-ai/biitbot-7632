          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Timestamp"
                  text={data?.timestamp ? format(parseISO(data?.timestamp as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Platform" text={data?.platform} />
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Message Sender Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_message_sender_idTouser?.id}`}>
                        {data?.user_message_sender_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Message Receiver Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_message_receiver_idTouser?.id}`}>
                        {data?.user_message_receiver_idTouser?.email}
                      </Link>
                    }
                  />
                )}
