...
import { getRolePermissionById } from 'apiSdk/role-permissions';
import { RolePermissionInterface } from 'interfaces/role-permission';

function RolePermissionViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('role_permission', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/role-permissions/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('role', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Role"
                    text={
                      <Link as={NextLink} href={`/roles/view/${data?.role?.id}`}>
                        {data?.role?.name}
                      </Link>
                    }
                  />
                ...
                {hasAccess('permission', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Permission"
                    text={
                      <Link as={NextLink} href={`/permissions/view/${data?.permission?.id}`}>
