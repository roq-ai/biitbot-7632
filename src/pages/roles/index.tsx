...
type ColumnType = ColumnDef<RoleInterface, unknown>;
...
export function RoleListPage(props: RoleListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRoles({
        relations: [, 'role_permission.count', 'user_role.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RoleInterface>>(
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('role_permission', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('user_role', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_role',
          header: 'User Role',
          accessorKey: 'user_role',
          cell: ({ row: { original: record } }: any) => record?._count?.user_role,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/roles/view/${record.id}`} passHref legacyBehavior>
                <Button
