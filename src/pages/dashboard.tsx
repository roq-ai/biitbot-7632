...
import { PermissionListPage as ShowCaseList } from 'pages/permissions';

function DashboardPage() {
  const { session } = useSession();

  const roleStat = useSWR<PaginatedInterface<RoleInterface>>(
    () => `/roles`,
    () => getRoles({ limit: 1, offset: 0 }),
  );

  const permissionStat = useSWR<PaginatedInterface<PermissionInterface>>(
    () => `/permissions`,
    () => getPermissions({ limit: 1, offset: 0 }),
  );

  const messageStat = useSWR<PaginatedInterface<MessageInterface>>(
    () => `/messages`,
    () => getMessages({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Roles',
      result: roleStat?.data?.totalCount,
      isLoading: roleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Permissions',
      result: permissionStat?.data?.totalCount,
      isLoading: permissionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Messages',
      result: messageStat?.data?.totalCount,
      isLoading: messageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`permissions`} passHref legacyBehavior>
                  <Button
